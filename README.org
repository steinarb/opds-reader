...
** TODO Make the main datamodel be a Metadata subclass (add a field to hold the parsed OPDS structure), and parse all available metadata info
** TODO Add a download button to download the selected books (keep it simple: will download all formats for a book)
** TODO Fix line height after updates
